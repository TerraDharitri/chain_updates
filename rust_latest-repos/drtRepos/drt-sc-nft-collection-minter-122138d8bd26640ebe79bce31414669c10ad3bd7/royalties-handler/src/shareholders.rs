    ...
    + crate::token_balance::TokenBalanceModule
{
    ...
    fn claim_rewards(&self, entry_ids: MultiValueEncoded<usize>) {
        ...
        for entry_id in entry_ids {
            ...
            if reward_entry.rewa_amount > 0 {
                self.send().direct_rewa(&caller, &reward_entry.rewa_amount);
            }
            if !reward_entry.dcdt_payments.is_empty() {
                self.send()
                    .direct_multi(&caller, &reward_entry.dcdt_payments);
            }
        }
    }
    ...
    fn get_claimable_tokens_for_reward_entry(
        ...
        if reward_entry.rewa_amount > 0 {
            result.push((RewaOrDcdtTokenIdentifier::rewa(), reward_entry.rewa_amount).into());
        }
        for p in &reward_entry.dcdt_payments {
            result.push(
                (
