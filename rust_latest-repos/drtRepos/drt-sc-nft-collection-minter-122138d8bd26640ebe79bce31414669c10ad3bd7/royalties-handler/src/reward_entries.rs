...
pub struct RewardEntry<M: ManagedTypeApi> {
    ...
    pub dcdt_payments: PaymentsVec<M>,
}
    ...
    crate::common_storage::CommonStorageModule + crate::token_balance::TokenBalanceModule
{
    ...
    fn create_new_reward_entry(&self) {
        ...
        let mut rewa_amount = BigUint::zero();
        let mut dcdt_payments = PaymentsVec::new();
        for token_id in self.known_tokens().iter() {
            ...
            if token_id.is_rewa() {
                rewa_amount = amount_per_holder;
            } else {
                dcdt_payments.push(DcdtTokenPayment::new(
        ...
        if rewa_amount > 0 || !dcdt_payments.is_empty() {
