...
use constants::*;
use dharitri_sc::types::{ManagedBuffer, ManagedByteArray, MultiValueEncoded};
...
fn create_brands_test() {
    ...
    nm_setup
        ...
        .execute_query(&nm_setup.nm_wrapper, |sc| {
            ...
            assert_eq!(result.nft_token_id, expected_token_id.unwrap_dcdt());
...
fn buy_random_nft_test() {
    ...
    nm_setup
        ...
        .execute_tx(&owner_addr, &nm_setup.nm_wrapper, &rust_biguint!(0), |sc| {
            ...
            let (rewa_amt, other_payments) = result.into_tuple();

            assert_eq!(rewa_amt, managed_biguint!(3 * FIRST_MINT_PRICE_AMOUNT));
            assert!(other_payments.is_empty());
        })
        .assert_ok();

    let owner_balance_before = OWNER_REWA_BALANCE - 2 * ISSUE_COST;
    let expected_balance = owner_balance_before + 3 * FIRST_MINT_PRICE_AMOUNT;
    nm_setup
        .b_mock
        .check_rewa_balance(&owner_addr, &rust_biguint!(expected_balance));
