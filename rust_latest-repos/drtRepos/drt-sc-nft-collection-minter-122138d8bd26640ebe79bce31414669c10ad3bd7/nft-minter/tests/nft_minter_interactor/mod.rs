...
macro_rules! managed_token_id {
    ($bytes:expr) => {{
        if $bytes
            == dharitri_sc::types::RewaOrDcdtTokenIdentifier::<DebugApi>::REWA_REPRESENTATION
        {
            dharitri_sc::types::RewaOrDcdtTokenIdentifier::rewa()
        } else {
            let ___token_id___ = dharitri_sc::types::TokenIdentifier::from_dcdt_bytes($bytes);
            dharitri_sc::types::RewaOrDcdtTokenIdentifier::dcdt(___token_id___)
        }
    }};
}
    ...
    NftMinterObjBuilder: 'static + Copy + Fn() -> nft_minter::ContractObj<DebugApi>,
{
    pub fn new(builder: NftMinterObjBuilder) -> Self {
        ...
        let owner_address = b_mock.create_user_account(&rust_biguint!(OWNER_REWA_BALANCE));
        let first_user_address = b_mock.create_user_account(&rust_biguint!(USER_REWA_BALANCE));
        let second_user_address = b_mock.create_user_account(&rust_biguint!(USER_REWA_BALANCE));
        let nm_wrapper =
            b_mock.create_sc_account(&rust_zero, Some(&owner_address), builder, "nft minter path");

        // init DCDT System SC mock
        b_mock.create_sc_account_fixed_address(
            &Address::from(DCDT_SYSTEM_SC_ADDRESS_ARRAY),
            &rust_zero,
            None,
            dcdt_system_sc_mock::contract_obj,
            "DCDT system SC mock path",
        );
    ...
    pub fn create_default_brands(&mut self) {
        ...
        self.b_mock.set_dcdt_local_roles(
    ...
    NftMinterObjBuilder: 'static + Copy + Fn() -> nft_minter::ContractObj<DebugApi>,
{
    ...
    pub fn call_buy_random_nft(
        ...
        if payment_token == REWA_TOKEN_ID {
            self.b_mock.execute_tx(
                buyer_address,
                &self.nm_wrapper,
