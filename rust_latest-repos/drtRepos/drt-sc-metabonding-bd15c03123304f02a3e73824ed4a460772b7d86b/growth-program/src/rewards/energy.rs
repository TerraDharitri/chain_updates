...
pub const ENERGY_FOUR_YEARS_: u64 = 4 * 360;
pub const _AMOUNT_FOR_APR_MATH: u64 = 10_000_000_000;
    ...
    + energy_query::EnergyQueryModule
{
    ...
    // Here the base investment for this APR is  locked for 4 years.
    #[only_owner]
    ...
    fn get_initial_rdpe(&self) -> BigUint {
        let moa_amount = BigUint::from(_AMOUNT_FOR_APR_MATH) * PRECISION;
        let moa_token_id = self.get_base_token_id();
        let moa_price = self.get_usdc_value(moa_token_id, moa_amount.clone(), DAY_IN_SECONDS);
        let apr = self.first_week_apr().get();
        let num = apr * moa_price * PRECISION * PRECISION;
