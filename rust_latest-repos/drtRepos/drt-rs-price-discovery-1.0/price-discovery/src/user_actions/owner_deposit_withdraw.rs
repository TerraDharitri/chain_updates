    ...
    + dharitri_sc_modules::default_issue_callbacks::DefaultIssueCallbacksModule
{
    ...
    fn owner_withdraw(&self, withdraw_amount: BigUint) -> DcdtTokenPayment {
        let phase = self.get_current_phase();
        self.require_owner_deposit_withdraw_allowed(&phase);
        ...
        self.send()
            .direct_dcdt(&caller, &launched_token_id, 0, &withdraw_amount);

        self.emit_owner_withdraw_event(&withdraw_amount);

        DcdtTokenPayment::new(launched_token_id, 0, withdraw_amount)
    }
}
