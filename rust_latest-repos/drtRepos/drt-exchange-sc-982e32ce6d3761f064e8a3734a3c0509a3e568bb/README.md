...
- [Dharitrix Smart Contracts](#dharitrix-smart-contracts)
  ...
  - [Locked Asset Contracts](#locked-asset-contracts)
    ...
    - [Locked  Factory Contract](#locked-moa-factory-contract)
...
In order to gain users trust, the liquidity inside the DEX must be somewhat stable. To achieve that, Farm contracts come in place to incentivise liquidity providers to lock their LP tokens in exchange for  rewards. Rewards are generated per block and their rate is configurable in sync with the  tokenomics.

### Farm with Lock Contract

Works the same as the regular Farm with the exception that it does not generate  as rewards. Instead, it generates Locked , with the help of the Factory SC. The reason one would choose to go with the locked rewards (LKMOA) instead of the regular rewards () is that the reward emission rate (reward per block rate) is bigger, meaning the APR is higher.
...
This smart contract is used for distributing Locked  to the community. It receives information from the owner and offers the users the possibility to claim the configured amount of tokens. It was used only once and might not be used in the future since we think there are better ways to achieve same functionality and results.

### DEX Proxy Contract

This smart contract allows users to interact with the DEX using Locked  as fungible  for certain operations, such as adding liquidity and entering Farm contracts.

### Locked  Factory Contract

Locked  is a Meta DCDT. Since there can be only one address (per shard) that can hold the role of creating a Meta DCDT, this contract was created so the multiple other contracts that need to create Locked  can just request them from only one place.
