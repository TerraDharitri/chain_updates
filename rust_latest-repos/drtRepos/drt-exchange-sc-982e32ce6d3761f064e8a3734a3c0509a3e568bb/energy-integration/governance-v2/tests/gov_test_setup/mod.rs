...
use dharitri_sc::{
    ...
    types::{Address, BigInt, DcdtLocalRole, ManagedVec, MultiValueEncoded},
};
...
pub const MIN_FEE_FOR_PROPOSE: u64 = 1_000_000_000; // 1B 
...
pub const _TOKEN_ID: &[u8] = b"-123456";
pub const X_TOKEN_ID: &[u8] = b"X-123456";
    ...
    GovBuilder: 'static + Copy + Fn() -> governance_v2::ContractObj<DebugApi>,
{
    pub fn new(gov_builder: GovBuilder) -> Self {
        ...
        b_mock
            .execute_tx(&owner, &gov_wrapper, &rust_zero, |sc| {
                sc.fee_token_id().set(managed_token_id!(_TOKEN_ID));
            })
            .assert_ok();

        let vote_nft_roles = [DcdtLocalRole::Mint, DcdtLocalRole::Burn];
        b_mock.set_dcdt_local_roles(gov_wrapper.address_ref(), _TOKEN_ID, &vote_nft_roles[..]);
