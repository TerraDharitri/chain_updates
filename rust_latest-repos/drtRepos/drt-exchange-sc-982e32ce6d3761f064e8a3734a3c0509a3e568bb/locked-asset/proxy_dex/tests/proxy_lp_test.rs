...
fn add_remove_liquidity_proxy_test() {
    ...
    setup
        .b_mock
        .execute_dcdt_multi_transfer(&first_user, &setup.proxy_wrapper, &payments, |sc| {
...
fn wrapped_same_nonce_lp_token_merge_test() {
    ...
    setup
        .b_mock
        .execute_dcdt_multi_transfer(&first_user, &setup.proxy_wrapper, &tokens_to_merge, |sc| {
    ...
    let liquidity_token_amount = (&first_amount + &second_amount) * rust_biguint!(2u64); // parity 1 REWA -> 2 
...
fn wrapped_different_nonce_lp_token_merge_test() {
    ...
    setup
        .b_mock
        .set_dcdt_balance(&user, _TOKEN_ID, &user_balance);
    ...
    setup
        .b_mock
        .execute_dcdt_multi_transfer(&user, &setup.proxy_wrapper, &payments1, |sc| {
    ...
    setup
        .b_mock
        .execute_dcdt_multi_transfer(&user, &setup.proxy_wrapper, &payments2, |sc| {
            let output_lp_token = sc.add_liquidity_proxy(
                managed_address!(&pair_addr),
                managed_biguint!(locked_token_amount.to_u64().unwrap()),
                managed_biguint!(other_token_amount.to_u64().unwrap()),
    ...
    setup
        .b_mock
        .execute_dcdt_multi_transfer(&user, &setup.proxy_wrapper, &tokens_to_merge, |sc| {
    ...
    let expected_locked_token_balance =
        &(locked_token_amount - min_locked_lp_token_amount) * rust_biguint!(2); // parity 1 REWA -> 2 
