...
use dharitri_sc::{
    ...
    types::{BigInt, DcdtLocalRole, DcdtTokenPayment},
};
...
fn farm_proxy_actions_test() {
    ...
    setup.b_mock.check_nft_balance(
        ...
        Some(&WrappedFarmTokenAttributes::<DebugApi> {
            proxy_farming_token: DcdtTokenPayment {
...
fn farm_with_wrapped_lp_test() {
    ...
    setup.b_mock.set_dcdt_local_roles(
        setup.farm_locked_wrapper.address_ref(),
        LP_TOKEN_ID,
        &[DcdtLocalRole::Burn],
    );
    ...
    setup.b_mock.check_nft_balance(
        ...
        &(&expected_lp_token_amount / 2u64 - &penalty_amount),
        Some(&WrappedLpTokenAttributes::<DebugApi> {
            locked_tokens: DcdtTokenPayment::new(
...
fn different_farm_locked_token_nonce_merging_test() {
    ...
    setup
        .b_mock
        .set_dcdt_balance(&first_user, _TOKEN_ID, &user_balance);
...
#[test]
fn increase_proxy_farm_lkmoa_energy() {
    let mut setup = ProxySetup::new(
        proxy_dex::contract_obj,
        pair::contract_obj,
        farm_with_locked_rewards::contract_obj,
...
fn total_farm_position_migration_mechanism_test() {
    ...
    setup
        .b_mock
        .execute_dcdt_multi_transfer(&first_user, &setup.proxy_wrapper, &enter_payments, |sc| {
            sc.enter_farm_proxy_endpoint(managed_address!(&farm_addr), OptionalValue::None);
        })
        .assert_ok();
    ...
    setup
        .b_mock
        .execute_dcdt_multi_transfer(&first_user, &setup.proxy_wrapper, &merge_payments, |sc| {
            sc.merge_wrapped_farm_tokens_endpoint(managed_address!(&farm_addr));
        })
        .assert_ok();
