    ...
    + legacy_token_decode_module::LegacyTokenDecodeModule
{
    ...
    ///     NOTE: The SC also needs the DCDTLocalMint and DCDTLocalBurn roles for this token.
    ...
    ///     a migrated old LKMOA token will be locked for after the average is calculated
    ...
    fn unlock_tokens_endpoint(&self) -> DcdtTokenPayment {
        self.require_not_paused();
        ...
        let mut output_payment = DcdtTokenPayment::new(base_asset, 0, BigUint::zero());
        ...
        self.update_energy(&caller, |energy: &mut Energy<Self::Api>| {
            ...
            for payment in &payments {
                ...
                output_payment.merge_with(self.to_dcdt_payment(unlocked_tokens));
            }
        });
    ...
    fn extend_lock_period(&self, lock_epochs: Epoch, user: ManagedAddress) -> DcdtTokenPayment {
        self.require_not_paused();
        self.require_is_listed_lock_option(lock_epochs);
