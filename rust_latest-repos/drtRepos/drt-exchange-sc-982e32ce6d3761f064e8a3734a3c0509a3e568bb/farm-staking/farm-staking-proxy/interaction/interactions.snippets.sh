...
# Deploy RIDE Proxy Staking Farm from  rewards
# $ deployProxyStakeFarmContract $LP_FARM__ADDRESS $STAKING_FARM_ADDRESS $LP_PAIR_ADDRESS $STAKING_TOKEN_ID $LP_FARM__TOKEN_ID $STAKING_FARM_TOKEN_ID $LP_TOKEN_ID

# Deploy RIDE Proxy Staking Farm from LKMOA rewards
# $ deployProxyStakeFarmContract $LP_FARM_LKMOA_ADDRESS $STAKING_FARM_ADDRESS $LP_PAIR_ADDRESS $STAKING_TOKEN_ID $LP_FARM_LKMOA_TOKEN_ID $STAKING_FARM_TOKEN_ID $LP_TOKEN_ID

# Issue DualYield Tokens
# $ registerDualYieldToken $PROXY_STAKING_FARM_ MetaStakedRide METARIDE 18
# $ registerDualYieldToken $PROXY_STAKING_FARM_LKMOA MetaStakedRideLK METARIDELK 18

# Set local roles for DualYield Tokens
# $ setLocalRolesDualYieldToken $PROXY_STAKING_FARM_
# $ setLocalRolesDualYieldToken $PROXY_STAKING_FARM_LKMOA
...
LP_FARM__TOKEN_ID="" # Fill in token ID
LP_FARM_LKMOA_TOKEN_ID="" # Fill in token ID
LP_TOKEN_ID="" # Fill in token ID

LP_PAIR_ADDRESS="" # Fill in address
LP_FARM__ADDRESS="" # Fill in address
LP_FARM_LKMOA_ADDRESS="" # Fill in address
STAKING_FARM_ADDRESS="" # Fill in address

PROXY_STAKING_FARM_="" # Fill in address after deploy
PROXY_STAKING_FARM_LKMOA="" # Fill in address after deploy
...
deployProxyStakeFarmContract() {
    lp_farm_address="0x$(drtpy wallet bech32 --decode $1)"
    staking_farm_address="0x$(drtpy wallet bech32 --decode $2)"
    pair_address="0x$(drtpy wallet bech32 --decode $3)"
    ...
    ADDRESS=$(drtpy data parse --file="deploy-proxy-stake-farm-internal.interaction.json" --expression="data['contractAddress']")
...
upgradeProxyStakeFarmContract() {
    ...
    drtpy --verbose contract upgrade $8 --recall-nonce \
        --pem=${WALLET_PEM} \
        --gas-limit=200000000 \
        --proxy=${PROXY} --chain=${CHAIN_ID} \
        --metadata-payable \
