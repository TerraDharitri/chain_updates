...
pub mod fuzz_data_tests {
    ...
    use dharitri_sc::types::{Address, BigUint, DcdtLocalRole};
    ...
    pub const WREWA_TOKEN_ID: &[u8] = b"WREWA-abcdef";
    pub const _TOKEN_ID: &[u8] = b"-abcdef";
    ...
    pub const _FARM_TOKEN_ID: &[u8] = b"FARM-abcdef";
    pub const MIN_FARMING_EPOCHS: u64 = 2;
    pub const FARM_PENALTY_PERCENT: u64 = 10;
    pub const OWNER_REWA_BALANCE: u64 = 100_000_000;
    pub const USER_TOTAL__TOKENS: u64 = 100_000_000_000;
    pub const USER_TOTAL_WREWA_TOKENS: u64 = 100_000_000_000;
        ...
        PriceDiscObjBuilder: 'static + Copy + Fn() -> price_discovery::ContractObj<DebugApi>,
    {
        pub fn new(
            ...
            let rewa_amount = rust_biguint!(OWNER_REWA_BALANCE);

            let rng = StdRng::seed_from_u64(seed);
            let fuzz_args = FuzzDexExecutorInitArgs::new();
            let statistics = EventsStatistics::new();
            let mut blockchain_wrapper = BlockchainStateWrapper::new();
            let owner_addr = blockchain_wrapper.create_user_account(&rewa_amount);

            let mut users = vec![];

            for i in 1..=fuzz_args.num_users {
                let user_address = blockchain_wrapper.create_user_account(&rewa_amount);
                blockchain_wrapper.set_dcdt_balance(
                    &user_address,
                    WREWA_TOKEN_ID,
                ...
                blockchain_wrapper.set_dcdt_balance(
                    &user_address,
                    _TOKEN_ID,
                    &rust_biguint!(USER_TOTAL__TOKENS),
                );
                blockchain_wrapper.set_dcdt_balance(
                    &user_address,
                    BUSD_TOKEN_ID,
        ...
        FarmObjBuilder: 'static + Copy + Fn() -> farm::ContractObj<DebugApi>,
    {
        ...
        let farming_token_roles = [DcdtLocalRole::Mint, DcdtLocalRole::Burn];
        blockchain_wrapper.set_dcdt_local_roles(
            farm_wrapper.address_ref(),
            farming_token,
            &farming_token_roles[..],
        ...
        let reward_token_roles = [DcdtLocalRole::Mint, DcdtLocalRole::Burn];
        blockchain_wrapper.set_dcdt_local_roles(
            farm_wrapper.address_ref(),
            reward_token,
            &reward_token_roles[..],
