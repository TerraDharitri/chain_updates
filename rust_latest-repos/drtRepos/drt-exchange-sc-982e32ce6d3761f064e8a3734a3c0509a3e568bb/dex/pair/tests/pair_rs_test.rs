...
use dharitri_sc::{
    ...
    types::{BigUint, DcdtLocalRole, MultiValueEncoded},
};
...
fn test_swap_fixed_input() {
    ...
    pair_setup.swap_fixed_input(WREWA_TOKEN_ID, 1_000, _TOKEN_ID, 900, 996);
}
...
fn test_swap_fixed_output() {
    ...
    pair_setup.swap_fixed_output(WREWA_TOKEN_ID, 1_000, _TOKEN_ID, 900, 96);
}
...
fn test_perfect_swap_fixed_output() {
    ...
    pair_setup.swap_fixed_output(WREWA_TOKEN_ID, 1_000, _TOKEN_ID, 996, 0);
    pair_setup.b_mock.check_dcdt_balance(
        &pair_setup.user_address,
        WREWA_TOKEN_ID,
        &(rust_biguint!(USER_TOTAL_WREWA_TOKENS - token_amount - 1_000)),
    );
    pair_setup.b_mock.check_dcdt_balance(
        &pair_setup.user_address,
        _TOKEN_ID,
        &(rust_biguint!(USER_TOTAL_WREWA_TOKENS - token_amount + 996)),
    );
}
...
fn test_locked_asset() {
    ...
    pair_setup.b_mock.set_dcdt_local_roles(
    ...
    pair_setup.b_mock.check_nft_balance(
        ...
        Some(&LockedTokenAttributes::<DebugApi> {
            original_token_id: managed_token_id_wrapped!(WREWA_TOKEN_ID),
            original_token_nonce: 0,
            unlock_epoch: 10,
        }),
    );

    let user_wrewa_balance_before =
        pair_setup
            .b_mock
            .get_dcdt_balance(&pair_setup.user_address, WREWA_TOKEN_ID, 0);
    ...
    pair_setup.b_mock.check_dcdt_balance(
        &pair_setup.user_address,
        WREWA_TOKEN_ID,
        &(user_wrewa_balance_before + rust_biguint!(996)),
    );
}
...
fn add_liquidity_through_simple_lock_proxy() {
    ...
    pair_setup.b_mock.check_nft_balance(
        ...
        &rust_biguint!(2_000_000),
        Some(&LockedTokenAttributes::<DebugApi> {
            original_token_id: managed_token_id_wrapped!(_TOKEN_ID),
            original_token_nonce: 0,
            unlock_epoch: 15,
        }),
    );
    ...
    // add liquidity through simple-lock SC - one locked (WREWA) token, one unlocked ()
    let transfers = vec![
        TxTokenTransfer {
        ...
        TxTokenTransfer {
            token_identifier: _TOKEN_ID.to_vec(),
            nonce: 0,
            value: rust_biguint!(500_000),
        },
    ];
    ...
    pair_setup.b_mock.check_nft_balance(
        ...
        Some(&LpProxyTokenAttributes::<DebugApi> {
            ...
            second_token_id: managed_token_id!(_TOKEN_ID),
    ...
    let user_moa_balance_before =
        pair_setup
            .b_mock
            .get_dcdt_balance(&pair_setup.user_address, _TOKEN_ID, 0);
...
fn fees_collector_pair_test() {
    ...
    pair_setup
        ...
        .execute_tx(
            ...
            |sc| {
                ...
                tokens.push(managed_token_id!(WREWA_TOKEN_ID));
                tokens.push(managed_token_id!(_TOKEN_ID));
    ...
    pair_setup.swap_fixed_input(WREWA_TOKEN_ID, 100_000, _TOKEN_ID, 900, 90_669);
    ...
    pair_setup
        ...
        .execute_query(&fees_collector_wrapper, |sc| {
            assert_eq!(
                sc.accumulated_fees(1, &managed_token_id!(WREWA_TOKEN_ID))
                    .get(),
                managed_biguint!(25)
            );
        })
