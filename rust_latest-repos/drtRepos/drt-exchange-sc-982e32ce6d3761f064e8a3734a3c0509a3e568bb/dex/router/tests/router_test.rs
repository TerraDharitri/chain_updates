...
use dharitri_sc::{
    ...
    types::{
        Address, RewaOrDcdtTokenIdentifier, DcdtLocalRole, ManagedAddress, ManagedVec,
        MultiValueEncoded,
    },
};
use pair::{
...
fn test_router_upgrade_pair() {
    ...
    b_mock.set_dcdt_balance(&user, USDC_TOKEN_ID, &rust_biguint!(USER_USDC_BALANCE));
...
fn test_multi_pair_swap() {
    ...
    router_setup.blockchain_wrapper.check_dcdt_balance(
    ...
    router_setup.multi_pair_swap(_TOKEN_ID, 100_000, &ops);
...
fn user_enable_pair_swaps_through_router_test() {
    ...
    b_mock
        .execute_dcdt_multi_transfer(&user, &pair_wrapper, &payments, |sc| {
...
fn user_enable_pair_swaps_fail_test() {
    ...
    b_mock.set_nft_balance(
        ...
        &LockedTokenAttributes::<DebugApi> {
            original_token_id: RewaOrDcdtTokenIdentifier::dcdt(managed_token_id!(LPUSDC_TOKEN_ID)),
            original_token_nonce: 0,
            unlock_epoch: current_epoch + MIN_LOCKED_PERIOD_EPOCHS,
        },
    );
