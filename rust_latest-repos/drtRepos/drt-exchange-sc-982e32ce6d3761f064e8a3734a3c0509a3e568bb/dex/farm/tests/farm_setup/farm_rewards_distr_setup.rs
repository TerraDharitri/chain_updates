...
use dharitri_sc::types::{Address, BigUint, DcdtLocalRole, ManagedAddress, MultiValueEncoded};
    ...
    FarmObjBuilder: 'static + Copy + Fn() -> farm::ContractObj<DebugApi>,
{
    ...
    pub fn exit_farm(
        ...
        b_mock.check_dcdt_balance(caller, _TOKEN_ID, &expected_moa_balance);
    }
    ...
    pub fn new_address_with_lp_tokens(&mut self, amount: RustBigUint) -> Address {
        ...
        blockchain_wrapper.set_dcdt_balance(&address, LP_TOKEN_ID, &amount);
        address
    }
}
