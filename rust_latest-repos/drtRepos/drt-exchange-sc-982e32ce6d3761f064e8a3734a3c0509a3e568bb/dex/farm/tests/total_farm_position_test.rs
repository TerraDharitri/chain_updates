...
use dharitri_sc::types::DcdtLocalRole;
...
fn no_boosted_rewards_penalty_for_no_energy_test() {
    ...
    // User unlocks X and has no energy
...
fn total_farm_position_owner_change_test() {
    ...
    farm_setup
        .b_mock
        .check_dcdt_balance(&second_user, REWARD_TOKEN_ID, &rust_biguint!(4507));
...
fn total_farm_position_through_simple_lock_test() {
    ...
    const WREWA_TOKEN_ID: &[u8] = b"WREWA-abcdef";
    const _TOKEN_ID: &[u8] = b"-abcdef"; // reward token ID
    ...
    farm_setup.b_mock.set_dcdt_local_roles(
