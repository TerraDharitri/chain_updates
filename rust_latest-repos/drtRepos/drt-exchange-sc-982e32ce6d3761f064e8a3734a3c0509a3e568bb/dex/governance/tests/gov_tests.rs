...
use dharitri_sc::types::{Address, DcdtLocalRole, DcdtTokenPayment, ManagedVec};
    ...
    PairMockObjBuilder: 'static + Copy + Fn() -> pair_mock::ContractObj<DebugApi>,
{
    ...
    blockchain_wrapper
        .execute_tx(&owner_addr, &pair_wrapper, &rust_zero, |sc| {
            sc.init(
                OptionalValue::Some(managed_token_id!(_TOKEN_ID)),
...
fn test_basic_reclaim() {
    ...
    let owner_address = gov_setup.owner_address.clone();
    gov_setup.blockchain_wrapper.check_dcdt_balance(
        &owner_address,
        _TOKEN_ID,
        &rust_biguint!(USER_TOTAL__TOKENS),
    );
...
#[test]
fn test_basic_propose_with_lpmoa() {
    let mut gov_setup = setup_gov(governance::contract_obj, pair_mock::contract_obj);
