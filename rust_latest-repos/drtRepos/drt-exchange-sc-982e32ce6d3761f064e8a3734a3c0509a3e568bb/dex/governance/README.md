...
This contract is meant to give  holders the oportunity to propose and vote actions, in a decentralized manner.
...
- governance_token_ids -> A list of tokens that users can vote with. Will contain , but also LP_, FARM_ and so on.
...
As described above, multiple tokens can be used to propose/vote. The value of each vote (weight) must be denominated in the amount of  tokens that a certain tokens holds behind it. The contract knows to use exec on dest ```updateAndGetTokensForGivenPositionWithSafePrice``` for each token's price provider in order to get the amount.
