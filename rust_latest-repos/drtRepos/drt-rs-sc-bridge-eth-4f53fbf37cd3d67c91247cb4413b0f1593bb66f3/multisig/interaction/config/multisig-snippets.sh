deployMultisig() {
    ...
    drtpy contract deploy --bytecode=${MULTISIG_WASM} --recall-nonce "${DRTPY_SIGN[@]}" \
    --gas-limit=200000000 \
    ...
    drtpy data store --key=address-testnet-multisig --value=${ADDRESS}
...
changeChildContractsOwnershipSafe() {
    ...
    drtpy contract call ${SAFE} --recall-nonce "${DRTPY_SIGN[@]}" \
...
addTokenToWhitelist() {
    ...
    drtpy contract call ${MULTISIG} --recall-nonce "${DRTPY_SIGN[@]}" \
    --gas-limit=60000000 --function="dcdtSafeAddTokenToWhitelist" \
...
removeTokenFromWhitelist() {
    ...
    drtpy contract call ${MULTISIG} --recall-nonce "${DRTPY_SIGN[@]}" \
    --gas-limit=60000000 --function="dcdtSafeRemoveTokenFromWhitelist" \
    --arguments str:${CHAIN_SPECIFIC_TOKEN} \
    --send --proxy=${PROXY} --chain=${CHAIN_ID}
}

dcdtSafeSetMaxTxBatchSize() {
    CHECK_VARIABLES MAX_TX_PER_BATCH MULTISIG

    drtpy contract call ${MULTISIG} --recall-nonce "${DRTPY_SIGN[@]}" \
    --gas-limit=30000000 --function="dcdtSafeSetMaxTxBatchSize" \
    --arguments ${MAX_TX_PER_BATCH} \
    --send --proxy=${PROXY} --chain=${CHAIN_ID}
}

dcdtSafeSetMaxTxBatchBlockDuration() {
    CHECK_VARIABLES MAX_TX_BLOCK_DURATION_PER_BATCH MULTISIG

    drtpy contract call ${MULTISIG} --recall-nonce "${DRTPY_SIGN[@]}" \
    --gas-limit=30000000 --function="dcdtSafeSetMaxTxBatchBlockDuration" \
...
pauseV2() {
    ...
    drtpy contract call ${MULTISIG_v2} --recall-nonce "${DRTPY_SIGN[@]}" \
    --gas-limit=40000000 --function="pause" \
    --send --proxy=${PROXY} --chain=${CHAIN_ID}
}
...
pauseDcdtSafe() {
    ...
    drtpy contract call ${MULTISIG} --recall-nonce "${DRTPY_SIGN[@]}" \
    --gas-limit=40000000 --function="pauseDcdtSafe" \
    --send --proxy=${PROXY} --chain=${CHAIN_ID}
}
...
unpauseDcdtSafe() {
    ...
    drtpy contract call ${MULTISIG} --recall-nonce "${DRTPY_SIGN[@]}" \
    --gas-limit=40000000 --function="unpauseDcdtSafe" \
    --send --proxy=${PROXY} --chain=${CHAIN_ID}
}
...
dcdtSafeSetMaxBridgedAmountForToken() {
    ...
    drtpy contract call ${MULTISIG} --recall-nonce "${DRTPY_SIGN[@]}" \
    --gas-limit=40000000 --function="dcdtSafeSetMaxBridgedAmountForToken" \
    --arguments str:${CHAIN_SPECIFIC_TOKEN} ${MAX} \
    --send --proxy=${PROXY} --chain=${CHAIN_ID}
}

multiTransferDcdtSetMaxBridgedAmountForToken() {
    CHECK_VARIABLES MAX_AMOUNT NR_DECIMALS_CHAIN_SPECIFIC CHAIN_SPECIFIC_TOKEN MULTISIG

    MAX=$(echo "scale=0; $MAX_AMOUNT*10^$NR_DECIMALS_CHAIN_SPECIFIC/1" | bc)
    drtpy contract call ${MULTISIG} --recall-nonce "${DRTPY_SIGN[@]}" \
    --gas-limit=40000000 --function="multiTransferDcdtSetMaxBridgedAmountForToken" \
    --arguments str:${CHAIN_SPECIFIC_TOKEN} ${MAX} \
    --send --proxy=${PROXY} --chain=${CHAIN_ID}
}

multiTransferDcdtSetMaxBridgedAmountForTokenWithRAWValue() {
...
setMultiTransferOnDcdtSafeThroughMultisig() {
    CHECK_VARIABLES MULTISIG

    drtpy contract call ${MULTISIG} --recall-nonce "${DRTPY_SIGN[@]}" \
    --gas-limit=60000000 --function="setMultiTransferOnDcdtSafe" \
    --send --proxy=${PROXY} --chain=${CHAIN_ID}
}

setDcdtSafeOnMultiTransferThroughMultisig() {
    CHECK_VARIABLES MULTISIG

    drtpy contract call ${MULTISIG} --recall-nonce "${DRTPY_SIGN[@]}" \
    --gas-limit=60000000 --function="setDcdtSafeOnMultiTransfer" \
    --send --proxy=${PROXY} --chain=${CHAIN_ID}
}
...
initSupplyMintBurn() {
  ...
  drtpy contract call ${MULTISIG} --recall-nonce "${DRTPY_SIGN[@]}" \
  --gas-limit=60000000 --function="initSupplyMintBurnDcdtSafe" \
...
syncValueWithEthereumDenom() {
  ...
  EXISTING_BURN=$(drtpy contract query ${SAFE} --proxy=${PROXY} --function getBurnBalances --arguments str:$TOKEN | jq '.[0].number')
  EXISTING_MINT=$(drtpy contract query ${SAFE} --proxy=${PROXY} --function getMintBalances --arguments str:$TOKEN | jq '.[0].number')
...
upgradeMultisig() {
    ...
    drtpy contract upgrade ${MULTISIG} --bytecode=${MULTISIG_WASM} --recall-nonce "${DRTPY_SIGN[@]}" \
      --gas-limit=100000000 --send \
      --arguments ${SAFE} ${MULTI_TRANSFER} ${BRIDGE_PROXY} \
      --outfile="upgrade-multisig-child-sc.json" --proxy=${PROXY} --chain=${CHAIN_ID} || return

    TRANSACTION=$(drtpy data parse --file="./upgrade-multisig-child-sc.json" --expression="data['emitted_tx']['hash']")
    ADDRESS=$(drtpy data parse --file="./upgrade-multisig-child-sc.json" --expression="data['contractAddress']")
