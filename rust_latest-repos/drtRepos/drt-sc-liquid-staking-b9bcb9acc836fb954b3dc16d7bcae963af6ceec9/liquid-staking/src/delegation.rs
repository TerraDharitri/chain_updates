...
use crate::basics::constants::{
    GasLimit, REWA_TO_WHITELIST, MAX_DELEGATION_ADDRESSES, MIN_BLOCKS_BEFORE_CLEAR_ONGOING_OP,
...
pub struct DelegationContractData<M: ManagedTypeApi> {
    ...
    pub rewa_in_ongoing_undelegation: BigUint<M>,
}
    ...
    + dharitri_sc_modules::default_issue_callbacks::DefaultIssueCallbacksModule
{
    ...
    fn whitelist_delegation_contract(
        ...
        let payment = self.call_value().rewa().clone_value();
        require!(payment == REWA_TO_WHITELIST, ERROR_BAD_WHITELIST_FEE);
        require!(
        ...
        let contract_data = DelegationContractData {
            ...
            rewa_in_ongoing_undelegation: BigUint::zero(),
        };
    ...
    fn whitelist_contract_callback(
        ...
        match result {
            ...
            ManagedAsyncCallResult::Err(_) => {
                self.send().direct_rewa(&caller, &REWA_TO_WHITELIST.into());
            }
        }
    ...
    fn get_delegation_contract_for_undelegate(
        ...
        while wrapped_last_node.is_some() {
            ...
            if delegation_contract_data.total_staked_from_ls_contract
                >= amount_to_undelegate + &delegation_contract_data.rewa_in_ongoing_undelegation
            {
                return delegation_address;
            }
        }
