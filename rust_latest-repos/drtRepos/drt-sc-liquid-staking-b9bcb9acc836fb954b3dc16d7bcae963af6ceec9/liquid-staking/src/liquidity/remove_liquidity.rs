...
use crate::{
    basics,
    basics::constants::{MIN_REWA_TO_DELEGATE, MIN_GAS_FOR_CALLBACK},
    basics::errors::{
    ...
    + basics::events::EventsModule
{
    ...
    fn remove_liquidity(&self) {
        ...
        require!(
            storage_cache.ls_token_id.is_valid_dcdt_identifier(),
        ...
        let rewa_to_unstake = self.pool_remove_liquidity(&payment.amount, &mut storage_cache);
        require!(
            rewa_to_unstake >= MIN_REWA_TO_DELEGATE,
            ERROR_INSUFFICIENT_UNSTAKE_AMOUNT
        );
        self.burn_ls_token(&payment.amount);

        self.call_undelegate(rewa_to_unstake, caller, payment.amount.clone());
    }
        ...
        match result {
            ManagedAsyncCallResult::Ok(()) => {
                ...
                delegation_contract_mapper.update(|contract_data| {
                    contract_data.total_staked_from_ls_contract -= &rewa_to_unstake;
                    contract_data.total_unstaked_from_ls_contract += &rewa_to_unstake;
                    contract_data.rewa_in_ongoing_undelegation -= &rewa_to_unstake;
                });

                let virtual_position = UnstakeTokenAttributes {
                    delegation_contract,
                    unstake_epoch: current_epoch,
                    unstake_amount: rewa_to_unstake,
                    unbond_epoch,
                };
            ...
            ManagedAsyncCallResult::Err(_) => {
                ...
                let ls_token_amount = self.pool_add_liquidity(&rewa_to_unstake, &mut storage_cache);
    ...
    fn call_undelegate(
        ...
        let delegation_contract = self.get_delegation_contract_for_undelegate(&rewa_to_unstake);

        let delegation_contract_mapper = self.delegation_contract_data(&delegation_contract);
        delegation_contract_mapper
            .update(|contract_data| contract_data.rewa_in_ongoing_undelegation += &rewa_to_unstake);

        let gas_for_async_call = self.get_gas_for_async_call();
        self.tx()
            .to(delegation_contract.clone())
            .typed(delegation_proxy::DelegationMockProxy)
            .undelegate(rewa_to_unstake.clone())
