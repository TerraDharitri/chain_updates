...
use crate::{
    basics,
    basics::constants::{MINIMUM_LIQUIDITY, MIN_REWA_TO_DELEGATE, MIN_GAS_FOR_CALLBACK},
    basics::errors::{
    ...
    + basics::events::EventsModule
{
    ...
    fn add_liquidity(&self) {
        ...
        require!(payment > MIN_REWA_TO_DELEGATE, ERROR_BAD_PAYMENT_AMOUNT);
    ...
    fn add_liquidity_callback(
        ...
        match result {
            ...
            ManagedAsyncCallResult::Err(_) => {
                self.send().direct_rewa(&caller, &staked_tokens);
