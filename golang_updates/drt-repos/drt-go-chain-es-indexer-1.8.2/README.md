...
Is a versatile component designed to enhance the data management capabilities of the `drt-go-chain` repository. 
...
- **Data Transformation**: `drt-go-chain-es-indexer` excels at transforming raw data generated by the `drt-go-chain` repository into a structured format suitable for indexing and storage.

- **Indexing**: The module indexes the transformed data within Elasticsearch, enhancing query performance and data retrieval.

- **Microservice Mode**: In microservice mode, drt-go-chain-es-indexer operates as a standalone microservice that communicates with drt-go-chain via WebSocket connections. This architecture promotes modularity and scalability.

- **Go Module Mode**: The `drt-go-chain` repository can include the module directly. This makes it easy to manage data without requiring a separate microservice.

### Running as a Separate Microservice

Running the `drt-go-chain-es-indexer` module as a **microservice** allows you to efficiently manage data indexing and storage while maintaining 
...
the `drt-go-chain` repository over `WebSocket` connections.

### Monitoring Endpoints

The `drt-go-chain-es-indexer` microservice provides monitoring endpoints that allow you to keep track of its health, performance, and other vital statistics.
...
[[HostDriversConfig]]
    ...
    # Possible values: json, gogo protobuf. Should be compatible with drt-go-chain-es-indexer config
    MarshallerType = "json"
...
Contributions to the `drt-go-chain-es-indexer` module are welcomed. Whether you're interested in improving its features, 
