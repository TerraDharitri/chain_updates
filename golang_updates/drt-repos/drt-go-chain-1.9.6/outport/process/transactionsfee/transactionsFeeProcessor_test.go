...
func TestPutFeeAndGasUsedScrWithRefund(t *testing.T) {
	...
	scr := &outportcore.SCRInfo{
		SmartContractResult: &smartContractResult.SmartContractResult{
			Nonce:          1005,
			SndAddr:        []byte("drt1yvesqqqqqqqqqqqqqqqqqqqqqqqqyvesqqqqqqqqqqqqqqqzlllsd5j0s2"),
...
func TestMoveBalanceWithSignalError(t *testing.T) {
	...
	initialTx := &outportcore.TxInfo{Transaction: &transaction.Transaction{
		...
		GasPrice: 1000000000,
		SndAddr:  []byte("drt1s8jr8e8hsvv7c9ehmshcjlpzf9ua5l50qeswa8feshrp6xlz9c7qpp0cgc"),
		RcvAddr:  []byte("drt1yvesqqqqqqqqqqqqqqqqqqqqqqqqyvesqqqqqqqqqqqqqqqzlllsd5j0s2"),
