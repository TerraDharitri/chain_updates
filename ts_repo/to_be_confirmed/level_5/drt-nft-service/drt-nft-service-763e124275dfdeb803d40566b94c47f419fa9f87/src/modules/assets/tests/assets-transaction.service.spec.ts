...
describe('Assets Transaction Service', () => {
  ...
  describe('burnQuantity', () => {
    ...
    it('returns burn transaction if after activation epoch', async () => {
      ...
      const expectedResult = {
        chainID: 'T',
        data: 'RENEVE5GVEJ1cm5ANDc0NTRlMmQ2NTY2NjYzNTMxNjNAMDNAMGE=',
        gasLimit: 200000,
        gasPrice: 1000000000,
        nonce: 0,
        options: undefined,
  ...
  describe('transferNft', () => {
    ...
    it('correctly handles large hex nonce like TEST-9e876f-0435', async () => {
      ...
      // DCDTNFTTransfer@TEST-9e876f@0435@01@destinationAddress
      expect(result.data).toContain('RENEVE5GVFRyYW5zZmVyQDU0NDU1MzU0MmQzOTY1MzgzNzM2NjZAMDQzNUAwMUA0YzYyNTcwMTEyNzk2NWEyMjdjNWRiZmI5M2QyMDQ5NzdmMjM0MTI4ZWMzOGU0ZTYzMTQyOTUyZDkyNGNjODc0');
    ...
    it('correctly handles problematic hex nonce like TEST-753c62-01f6', async () => {
      ...
      expect(result.data).toContain('RENEVE5GVFRyYW5zZmVyQDU0NDU1MzU0MmQzNzM1MzM2MzM2MzJAMDFmNkAwMUA0YzYyNTcwMTEyNzk2NWEyMjdjNWRiZmI5M2QyMDQ5NzdmMjM0MTI4ZWMzOGU0ZTYzMTQyOTUyZDkyNGNjODc0');
      expect(result.chainID).toBe('T');
      expect(result.receiver).toBe(ownerAddress);
      expect(result.gasLimit).toBe(1216500);
    });
    ...
    it('correctly handles simple hex nonces like 01, 04', async () => {
      ...
      expect(result.data).toContain('RENEVE5GVFRyYW5zZmVyQDU0NDU1MzU0MmQzMTMyMzMzNDM1MzZAMDFAMDFANGM2MjU3MDExMjc5NjVhMjI3YzVkYmZiOTNkMjA0OTc3ZjIzNDEyOGVjMzhlNGU2MzE0Mjk1MmQ5MjRjYzg3NA==');
      expect(result.chainID).toBe('T');
      expect(result.receiver).toBe(ownerAddress);
    });
    ...
    it('correctly handles very large hex nonces', async () => {
      ...
      expect(result.data).toContain('RENEVE5GVFRyYW5zZmVyQDU0NDU1MzU0MmQzMTMyMzMzNDM1MzZAZmZmZkAwMUA0YzYyNTcwMTEyNzk2NWEyMjdjNWRiZmI5M2QyMDQ5NzdmMjM0MTI4ZWMzOGU0ZTYzMTQyOTUyZDkyNGNjODc0');
      expect(result.chainID).toBe('T');
      expect(result.receiver).toBe(ownerAddress);
    });
    ...
    it('correctly handles multi-digit hex nonces with leading zeros', async () => {
      ...
      expect(result.data).toContain('RENEVE5GVFRyYW5zZmVyQDU0NDU1MzU0MmQzMTMyMzMzNDM1MzZAYWJAMDFANGM2MjU3MDExMjc5NjVhMjI3YzVkYmZiOTNkMjA0OTc3ZjIzNDEyOGVjMzhlNGU2MzE0Mjk1MmQ5MjRjYzg3NA==');
      expect(result.chainID).toBe('T');
      expect(result.receiver).toBe(ownerAddress);
    });
