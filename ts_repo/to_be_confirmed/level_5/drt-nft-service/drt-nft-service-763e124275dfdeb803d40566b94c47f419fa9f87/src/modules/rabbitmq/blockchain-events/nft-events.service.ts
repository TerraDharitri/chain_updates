    ...
    for (let event of mintEvents) {
      switch (event.identifier) {
        ...
        case NftEventEnum.DCDTNFTUpdateAttributes:
          ...
          const collectionUpdateInfo = await this.drtApiService.getCollectionByIdentifierForQuery(
            updateTopics.collection,
            'fields=name,type',
          );
          if (collectionUpdateInfo?.type === NftTypeEnum.NonFungibleDCDT || collectionUpdateInfo?.type === NftTypeEnum.SemiFungibleDCDT) {
            await this.triggerCacheInvalidation(`${updateTopics.collection}-${updateTopics.nonce}`, CacheEventTypeEnum.AssetRefresh);
          }
          break;
      }
    }
