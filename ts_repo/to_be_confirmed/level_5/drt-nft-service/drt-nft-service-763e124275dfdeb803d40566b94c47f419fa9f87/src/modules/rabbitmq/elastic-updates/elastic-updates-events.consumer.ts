...
export class ElasiticUpdatesConsumer {
  ...
  async consumeMintBurnAndUpdateEvents(events: any) {
    const updateEvents = [
      NftEventEnum.DCDTNFTBurn,
      NftEventEnum.DCDTNFTUpdateAttributes,
      NftEventEnum.DCDTMetaDataUpdate,
      NftEventEnum.DCDTMetaDataRecreate,
    ];
    if (events.events && process.env.ENABLE_ELASTIC_UPDATES === 'true') {
      ...
      const burnAndUpdateNftAttributesEvents = events?.events?.filter((e: { identifier: NftEventEnum }) =>
        updateEvents.includes(e.identifier),
      );
