...
export class UpdateCampaignsAndTiers1651747398282 implements MigrationInterface {
  name = 'UpdateCampaignsAndTiers1651747398282';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`ALTER TABLE \`tiers\` ADD \`description\` varchar(255) NOT NULL`);
    await queryRunner.query(`ALTER TABLE \`campaigns\` ADD \`maxNftsPerTransaction\` int NOT NULL`);
    await queryRunner.query(`ALTER TABLE \`campaigns\` ADD \`verified\` int NOT NULL`);
    await queryRunner.query(`ALTER TABLE \`campaigns\` ADD \`description\` varchar(255) NOT NULL`);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`ALTER TABLE \`campaigns\` DROP COLUMN \`verified\``);
    await queryRunner.query(`ALTER TABLE \`campaigns\` DROP COLUMN \`maxNftsPerTransaction\``);
    await queryRunner.query(`ALTER TABLE \`tiers\` DROP COLUMN \`description\``);
