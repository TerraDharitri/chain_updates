...
export class AddIndexesAndConstrainsToStrings1636537187367 implements MigrationInterface {
  name = 'AddIndexesAndConstrainsToStrings1636537187367';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query('ALTER TABLE `orders` MODIFY COLUMN `priceToken` varchar(20) NOT NULL');
    await queryRunner.query('ALTER TABLE `orders` MODIFY COLUMN `priceAmount` varchar(62) NOT NULL');
    await queryRunner.query('ALTER TABLE `orders` MODIFY COLUMN  `status` varchar(8) NOT NULL');
    await queryRunner.query('ALTER TABLE `orders` MODIFY COLUMN  `boughtTokensNo` varchar(62) NULL');
    await queryRunner.query('ALTER TABLE `auctions` MODIFY COLUMN  `status` varchar(10) NOT NULL');
    await queryRunner.query('ALTER TABLE `auctions` MODIFY COLUMN  `startDate` int NOT NULL');
    await queryRunner.query('ALTER TABLE `auctions` MODIFY COLUMN  `endDate` int NOT NULL');
    await queryRunner.query('CREATE INDEX `order_status` ON `orders` (`status`)');
