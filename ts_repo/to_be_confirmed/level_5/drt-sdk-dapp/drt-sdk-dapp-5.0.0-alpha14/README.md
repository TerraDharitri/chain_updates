...
If you need only the core behaviour, without the additional UI, you can create a project-specific `.npmrc` file to configure per-package installation behavior. This will skip the installation of `@terradharitri/sdk-dapp-ui`, but keep in mind that you may need to provide the UI components yourself.
Also, make sure you run your app on `https`, not `http`, otherwise some providers will not work.

```bash
## .npmrc
@terradharitri/sdk-dapp:omit-optional=true
## enable the option when needed with:
## @terradharitri/sdk-dapp:omit-optional=false
...
import { DrtTransactionsTable } from '@terradharitri/sdk-dapp-ui/dist/react';
...
```tsx
import { DrtFormatAmount } from '@terradharitri/sdk-dapp-ui/dist/react';
import { DrtFormatAmountPropsType } from '@terradharitri/sdk-dapp-ui/dist/types';
