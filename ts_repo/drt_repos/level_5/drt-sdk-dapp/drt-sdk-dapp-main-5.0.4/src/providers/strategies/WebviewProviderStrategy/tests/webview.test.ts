...
import { WebviewProvider } from '@terradharitri/sdk-webview-provider/out';

// Mock getSafeWindow
jest.mock('@terradharitri/sdk-webview-provider/out/helpers/getSafeWindow', () => ({
  getSafeWindow: jest.fn()
}));

// Mock getSafeDocument to prevent real DOM access
jest.mock(
  '@terradharitri/sdk-webview-provider/out/helpers/getSafeDocument',
  () => ({
...
describe('WebviewProvider.init', () => {
  ...
  it('should initialize using ReactNativeWebView', async () => {
    ...
    const getSafeWindow =
      require('@terradharitri/sdk-webview-provider/out/helpers/getSafeWindow').getSafeWindow;
