...
## Overview
This folder contains API call functions and modules for the Dharitri SDK DApp project. These functions handle communication with the Dharitri blockchain API to fetch account data, transaction information, economics data, and more.
...
**Main Functions**: Exports endpoint constants for various Dharitri API resources including:
...
**Purpose**: Fetches account information from the Dharitri API.
...
const account = await getAccountFromApi({
  address: 'drt1...',
  baseURL: 'https://api.dharitri.org'
});
```
